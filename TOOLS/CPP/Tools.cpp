...
int _RandVUKL(int min, int max)
{
	...
	std::mt19937 gen(rd());
	std::uniform_int_distribution<int> distribution(min, max - 1); // »спользуем max - 1, чтобы исключить верхнее значение
...
/*int _RandVUKL(int min, int max) {
	// »нициализируем генератор случайных чисел с использованием текущего времени
	std::srand(static_cast<unsigned int>(std::time(nullptr)));

	// √енерируем случайное число в диапазоне [min, max - 1]
...
int _RandVKL(int min, int max) {
	...
	// √енерируем случайное число в диапазоне [min, max]
...
{
	sleepBuffer += deltaTime; // Ќакапливаем врем€
	// ≈сли накопленное врем€ больше или равно времени ожидани€
	if (sleepBuffer >= wait_time) {
		sleepBuffer = 0.0f; // —брасываем накопленное врем€
		return true; // ¬рем€ ожидани€ истекло, возвращаем true
	}
	return false; // ¬рем€ ожидани€ не истекло, возвращаем false
}
...
bool _MkCFG(std::string& config_path) // EXAMPLE
{
	...
	if (outfile.is_open()) {
		...
		outfile << ";healthdiff разница здоровь€ до и после удара" << std::endl;
...
void _InitCFG(std::string& config_path) // EXAMPLE
{
	...
	//std::locale loc("C");
	//infile.imbue(loc); // дл€ парсинга 1.2f
